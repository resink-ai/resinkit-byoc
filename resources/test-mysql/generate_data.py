...
def generate_invitation(team_id, invited_by):
    return {
        ...
        'allowedDomains': json.dumps([fake.domain_name() for _ in range(random.randint(0, 3))])
    }
...
def insert_data(connection, table_name, data):
    try:
        ...
        final_values = [json.dumps(value) if isinstance(
            value, (list, dict)) else value for value in data.values()]
...
def main():
    ...
    for team in teams:
        ...
        for _ in range(random.randint(0, 2)):
            invitation = generate_invitation(
                team['id'], random.choice(users)['id'])
    ...
    for team in teams:
        for _ in range(random.randint(1, 3)):
            ...
            insert_data(connection, 't_data_connection_config',
                        data_connection)
