...
run_alembic_migrations() {
    ...
    if [[ -f "$RESINKIT_API_PATH/resinkit_api/db/alembic.ini" ]]; then
        $UV_BIN --directory "$RESINKIT_API_PATH" run alembic -c "$RESINKIT_API_PATH/resinkit_api/db/alembic.ini" upgrade head || true
    else
        echo "[RESINKIT] Alembic ini file not found, skipping migrations..."
    fi
}
